...
class FourTrackEvents:
    ...
    def __init__(self):
        ...
        self._four_tracks_zq_mask = self._four_tracks_mask[
            self.orig_tracks.groupby("entry").sum().T_Q == 0
        ]

        # non zero total charge mask for 4 tracks events
        self._four_tracks_nzq_mask = self._four_tracks_mask[
            self.orig_tracks.groupby("entry").sum().T_Q != 0
        ]
    ...
    def ft_nzq_nTPC(self):
        ...
        _ft_nzq_nTPC = []
        for i in range(5):
            _ft_nzq_nTPC.append(self.GetTracksWithNTPC(self.four_tracks_nzq, i))
        return _ft_nzq_nTPC
